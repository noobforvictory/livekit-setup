...
A complete starter project for building voice AI apps with [LiveKit Agents for Python](https://github.com/livekit/agents). This checkout has been adapted to run against the local self-hosted LiveKit server in the parent workspace.

The starter project includes:

- A simple voice AI assistant, ready for extension and customization
- A voice AI pipeline using Deepgram STT, the OpenAI LLM API, and Azure Speech TTS directly, which works with self-hosted LiveKit
...
For this local workspace, use the short runbook first: [LOCAL_LIVEKIT.md](LOCAL_LIVEKIT.md).

### Local self-hosted setup for this workspace

Start the local LiveKit example from the parent directory:
...
In another terminal, install and configure the agent:
...
Set your provider keys in `.env.local`:
...
If your Azure Speech resource uses a host instead of key plus region, set `AZURE_SPEECH_HOST` instead.

The LiveKit values are already set for `livekit-server --dev`:
...
Download the local Silero VAD model, then run the agent:
...
For direct terminal testing, use:

```console
uv run python src/agent.py console
```

If you run this agent from inside a container instead of on the host, change `LIVEKIT_URL` to an address reachable from that container, such as `ws://172.17.0.1:7880`.
...
Before your first run, download the local Silero VAD model:
